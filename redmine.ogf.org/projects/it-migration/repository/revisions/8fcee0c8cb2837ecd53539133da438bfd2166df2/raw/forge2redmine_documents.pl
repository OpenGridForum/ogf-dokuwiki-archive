...
my $forge_root     = "/data/sourceforge_var/filestorage/";


# change the settings below to point to localhost for testing
...
sub inspect_folder ($$$)
{
  ...
  # right now we only handle the Documents subtree
...
sub handle_doc ($$$)
{
  ...
  # create the logical target dir in redmine's document tree
  my $folder_id = rmine_mkdir ($d_path);

  # get document info
  my $iquery = $forge->prepare ("SELECT * FROM document WHERE id = '$d_id'");
     $iquery->execute();

  # we should only see one match - check!  OTOH, so what...
  ...
  while ( my $irow = $iquery->fetchrow_hashref () )
  {
    ...
    my $tquery = $rmine->prepare (qq{
      SELECT * FROM dmsf_files WHERE name           = '$d_title' 
    ...
    # handle all versions for this document
    ...
    while ( my $vrow = $vquery->fetchrow_hashref () )
    {
      ...
      # handle the physical file this version points to
      ...
      # re-create the sourceforge naming scheme for phisical file storage
      my $s1     = substr ($f_raw, 0, 1);
      my $s2     = substr ($f_raw, 0, 2);
      my $s3     = substr ($f_raw, 0, 3);
      my $s4     = substr ($f_raw, 0, 4);
      my $src    = "$forge_root/$s1/$s2/$s3/$s4/$f_raw";

      # create a sensible local storage location
      my $tgt_r  = "$rmine_root";
      ...
      # fetch the file to local, if needed
      if ( ! -e $tgt )
      {
      ...
      my $tquery = $rmine->prepare (qq{
        SELECT * FROM dmsf_file_revisions WHERE name           = '$f_name' 
                                            AND title          = '$d_title' 
                                            AND project_id     = $rmine_pid 
      ...
      else
      {
        ...
        $rmine->do ($v_insert) 
          or die "Cannot create document version: $d_path/$d_title : $v_version : " . $rmine->errstr . "\n";
...
######################################################################
#
# make a dmsf directory hierarchy
#
sub rmine_mkdir ($)
{
  my $path  = shift;
